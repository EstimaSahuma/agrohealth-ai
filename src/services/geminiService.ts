...
import { issueType, severity, type PlantAnalysis } from "../types";
...
export const analyzePlantHealth = async (
    ...
    config: {
      ...
      responseSchema: {
        ...
        properties: {
          plantName: {
            type: Type.STRING,
            description: "Common name of the plant (e.g., Tomato, Corn).",
          },
          issueType: {
            type: Type.STRING,
            enum: [
          ...
          diagnosis: {
            type: Type.STRING,
            description:
              "Simple name of the problem (e.g., Aphids, Early Blight).",
          },
          symptoms: {
            type: Type.ARRAY,
          ...
          explanation: {
            type: Type.STRING,
            description:
              "A friendly explanation of what's happening to the plant.",
          },
          severity: {
            type: Type.STRING,
  ...
  if (!text) {
    throw new Error(
      "We couldn't get a clear answer. Please try again with a better photo.",
    );
  }
  ...
  try {
    ...
    throw new Error(
      "We had trouble reading the expert's notes. Please try again.",
    );
  }
};
